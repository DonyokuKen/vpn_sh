...
ipv6aku=$(ip addr show enp0s6 | grep "inet6\b" | awk '{print $2}' | cut -d/ -f1)
ipv4aku=$(ip addr show enp0s6 | grep "inet\b" | awk '{print $2}' | cut -d/ -f1)
...
ipcidr=$(ip -4 -o addr show enp0s6 | awk '{print $4}')
